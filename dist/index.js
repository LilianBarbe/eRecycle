...
(() => {
  ...
  function checkBreakpoint(x) {
    if (x.matches) {
      $("[data-swiper-component]").each(function(index) {
        let loopMode = false;
  ...
  var matchMediaDesktop = window.matchMedia("(max-width: 992px)");
  checkBreakpoint(matchMediaDesktop);
  matchMediaDesktop.addListener(checkBreakpoint);
  $(".u-span-wrap").each(function(index) {
    let relatedEl = $(".u-span-el").eq(index);
